...
public class Problem746 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(minCostClimbingStairs(new int[]{1, 10, 10, 5, 3}));
    ...
    static List<Integer> listOfSums;
    public static int minCostClimbingStairs1(int[] cost) {
        ...
        climbStairCase(cost, 0, sum);
        climbStairCase(cost, 1, sum);
        Collections.sort(listOfSums);
        return listOfSums.stream().findFirst().get();
    }

    private static void climbStairCase(int[] cost, int index, int sum) {
        if (cache.get(index) != null && cache.get(index) > 4) {
            return;
        }
        if (index >= cost.length) {
            listOfSums.add(sum);
            cache.put(index, cache.get(index) != null ? cache.get(index) + 1 : 1);
            Collections.sort(listOfSums);
            return;
        }
        sum += cost[index];
        if (listOfSums.size() != 0 && (sum > listOfSums.get(listOfSums.size() - 1) || listOfSums.contains(sum))) {
            return;
        }
        climbStairCase(cost, index + 1, sum);
        climbStairCase(cost, index + 2, sum);
    }
    ...
    public static int minCostClimbingStairs2(int[] cost) {
        ...
        while (index < cost.length - 1) {
            if (cost[index] < cost[index + 1]) {
                if (cost[index] + ifEnd(index + 2, cost) <= cost[index + 1]) {
                    sum += cost[index];
                    index++;
                } else {
                    sum += cost[index + 1];
    ...
    static int ifEnd(int index, int[] array) {
    ...
    public static int minCostClimbingStairs(int[] cost) {
        ...
        int index = 0, sum = 0;
        while (index < cost.length - 1) {
            if (index == cost.length - 4) {
                sum += Math.min(Math.min(cost[index] + cost[index + 2], cost[index + 1] + cost[index + 3]),
                    Math.min(cost[index] + cost[index + 3], cost[index + 1] + cost[index + 2]));
                break;
            }
            if (cost[index] < cost[index + 1]) {
                sum += cost[index];
...
//    private static int makesStep(int i, int i1, int[] cost) {
//
//    }
}
